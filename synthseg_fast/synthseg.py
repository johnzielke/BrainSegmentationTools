...
class Synthseg(torch.nn.Module):
    ...
    def __init__(self, *,
        ...
        if self.flip_indices is not None:
            if not isinstance(self.flip_indices, dict):
                raise ValueError()
        self.robust = robust
    ...
    def forward(self, image:torch.Tensor):
        ...
        if self.do_parcellation:
            ...
            if False and (segmentation[:,0,...] > 0.5).all():
                parcellation = torch.zeros(image.shape[0], n_labels_parcellation, *image.shape[2:], device=image.device)
                parcellation[:,0,...] = 1
            else:
                parcellation = self.parcellation_model(self._prepare_parcellation_input(image=image, segmentation=segmentation))

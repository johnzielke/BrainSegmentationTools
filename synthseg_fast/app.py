...
from brain_segmentation_tools import utils, postprocessing, preprocessing
...
class Application:
    ...
    BATCH_SIZE = 1 # The batch size is fixed to 1 and the prediction does currently not support bigger batches
    def __post_init__(self):
        ...
        if (not self.fast) & (not self.robust):
            self.labels_segmentation, self.flip_indices, unique_idx = utils.get_flip_indices(labels_segmentation, self.n_neutral_labels)
        else:
    ...
    def synthseg_model(self):
        ...
        if not self.no_compile:
            self._synthseg_model = torch.compile(self._synthseg_model)
        return self._synthseg_model
    ...
    def synthstrip_model(self):
        ...
        self._synthstrip_model.load_state_dict(torch.load(synthstrip_model_file, weights_only=True))
        self._synthstrip_model = self._synthstrip_model.to(self.device,dtype=self.dtype)
        self._synthstrip_model.eval()
        if not self.no_compile:
            self._synthstrip_model = torch.compile(self._synthstrip_model)
    ...
    def predict_synthseg_batch(self, image_tensor, brain_mask=None):
        ...
        brain_mask: An optional batch of brain masks to crop the input images. Shape (H, W, D)
        """
        if brain_mask is not None:
            original_shape = image_tensor.shape[2:]
            bbox_start,bbox_end = generate_spatial_bounding_box(brain_mask[None], margin=5)
            image_tensor = image_tensor[:, :, bbox_start[0]:bbox_end[0], bbox_start[1]:bbox_end[1], bbox_start[2]:bbox_end[2]]
        segmentation, parcellation = self.synthseg_model(image_tensor.to(self.device,dtype=self.dtype))

        results = []
        for i in range(segmentation.shape[0]):
            # seg_ = postprocessing.undo_padding(segmentation[i], n_levels=Synthseg.N_LEVELS)
            # if self.parcellation:
            #     parc_ = postprocessing.undo_padding(parcellation[i], n_levels=Synthseg.N_LEVELS)
            # else:
            #     parc_ = None
            seg_ = segmentation[i]
            if self.parcellation:
            ...
            if brain_mask is not None:
                ...
                segmentation_final_labels_ = torch.zeros(original_shape, dtype=segmentation_final_labels.dtype, device=segmentation_final_labels.device)
                segmentation_final_labels_[bbox_start[0]:bbox_end[0], bbox_start[1]:bbox_end[1], bbox_start[2]:bbox_end[2]] = segmentation_final_labels
                segmentation_final_labels = segmentation_final_labels_
            row = {}

            segmentation_final_labels = monai.data.MetaTensor(segmentation_final_labels, affine=seg_.affine)
    ...
    def predict_synthstrip_batch(self, data):
        ...
        for i in range(brain_mask.shape[0]):
            # brain_mask_ = postprocessing.undo_padding(brain_mask[i], n_levels=Synthseg.N_LEVELS)
            ...
            row = {}
            brain_mask = monai.data.MetaTensor(brain_mask, affine=brain_mask_.affine)
    ...
    @torch.inference_mode()
    def run(self, input_paths: str | list[str],*, segmentation_out: str | list[str] | None = None,  brain_mask_out: str | list[str] | None = None, data_root: str | None = None, callback: Callable[[dict[str,str]], None] | None = None, ids: list[str] | None = None, use_prog_bar: bool = True):
        ...
        if data_root is not None:
            if segmentation_out is not None:
                ...
                segmentation_out = [segmentation_out] * len(input_paths)
            if brain_mask_out is not None:
                ...
                brain_mask_out = [brain_mask_out] * len(input_paths)
        else:
            segmentation_out = segmentation_out if isinstance(segmentation_out, list) else [segmentation_out]
            brain_mask_out = brain_mask_out if isinstance(brain_mask_out, list) else [brain_mask_out]

        extended_input_paths = []
        extended_segmentation_out_paths = []  
        extended_brain_mask_out_paths = []
        if brain_mask_out is None:
            brain_mask_out = [None] * len(input_paths)
        if segmentation_out is None:
            segmentation_out = [None] * len(input_paths)

        for input_path, brain_mask_out_path, segmentation_out_path in zip(input_paths, brain_mask_out, segmentation_out):
            ...
            if input_path.is_dir():
                ...
                for extension in self.input_file_extensions:
                    ...
                    file_rel_paths = [Path(p.relative_to(input_path).as_posix().removesuffix(extension) + self.output_extension) for p in matched_files]
                ...
                if data_root is not None:
                    rel_path = Path(input_path.relative_to(data_root).as_posix().removesuffix(".nii.gz") + self.output_extension)
                    if segmentation_out_path is not None:
        ...
        if self.skip_existing:
            ...
            input_paths = [input_path for input_path, existing_output in zip(input_paths, existing_outputs) if not existing_output]
            segmentation_out_paths = [output_path for output_path, existing_output in zip(segmentation_out_paths, existing_outputs) if not existing_output]
            brain_mask_out_paths = [brain_mask_out_path for brain_mask_out_path, existing_output in zip(brain_mask_out_paths, existing_outputs) if not existing_output]
            print(f"Skipping {sum(existing_outputs)} existing segmentations")
        preprocessing_transform = preprocessing.get_pre_transforms(synthseg_divisible_k=Synthseg.SYNTHSEG_DIVISIBLE_K, device=self.device, synthstrip=do_brain_mask, synthseg=do_segmentation)
        dataset = utils.ErrorCatchingDataset(monai.data.Dataset([
            {"image": image.as_posix(), "output": output.as_posix() if output is not None else "", "brain_mask_output": brain_mask_out.as_posix() if brain_mask_out is not None else "", "id": id} for image, output, brain_mask_out, id in zip(input_paths, segmentation_out_paths, brain_mask_out_paths, ids)
        ],preprocessing_transform))
        assert self.BATCH_SIZE == 1, "Batch size must be 1 for the pipeline logic"
        dataloader = monai.data.ThreadDataLoader(dataset, batch_size=self.BATCH_SIZE,use_thread_workers=True,num_workers=4)
        saver = transforms.SaveImage(output_ext=self.output_extension,print_log=False)
        # with utils.ThreadPoolExecutorWithQueueSizeLimit(maxsize=2) as save_executor:
        with nullcontext(dataloader) if not use_prog_bar else tqdm(dataloader) as t:
            for data in t:
                # TODO If we are doing both segmentation and brain_mask, we could use the brain mask to crop the image
                ...
                if use_prog_bar:
                    t.set_description(input_image_path)
                brain_masks = None
                if do_brain_mask or self.crop_segmentation_input_to_brain_mask:
                    ...
                    if do_brain_mask:
                        for i in range(len(brain_masks)):
                            brain_mask_output_path = data["brain_mask_output"][i].removesuffix(".nii").removesuffix(".nii.gz")
                            Path(brain_mask_output_path).parent.mkdir(parents=True, exist_ok=True)
                            saver(brain_masks[i]["brain_mask"][None].cpu(), filename=brain_mask_output_path)
                ...
                if do_segmentation:
                    ...
                    for i in range(len(segmentations)):
                        ...
                        # brain_mask_output_path = data["brain_mask_output"][i].removesuffix(".nii").removesuffix(".nii.gz") if data["brain_mask_output"][i] is not None else None
                        Path(output_path).parent.mkdir(parents=True, exist_ok=True)
                        # save_executor.submit(saver,segmentations[i][None].cpu(), filename=output_path)
                        saver(segmentations[i]["segmentation"][None].cpu(), filename=output_path)
                        item_result["synthseg"] = output_path + self.output_extension
                        # if brain_mask_output_path is not None:
                        #     Path(brain_mask_output_path).parent.mkdir(parents=True, exist_ok=True)
                        #     saver(segmentations[i]["brain_mask"][None].cpu(), filename=brain_mask_output_path)
                if callback is not None:
...
if __name__ == "__main__":
    ...
    app.run(input_paths=example_file.as_posix(),
        segmentation_out_paths=output_file.as_posix())
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding='same', 
                 activation='elu', use_residual=False, conv_dropout=0, batch_norm=None,
...
def upsample_layer(channels, factor=2):
    layer = nn.ConvTranspose3d(channels, channels, kernel_size=factor, stride=factor,groups=channels,bias=False)
    #Set weights to 1
...
class UNet(nn.Module):
    def __init__(self, nb_features, in_channels, nb_levels, conv_size, nb_labels,
                 feat_mult=1, pool_size=2, padding='same', activation='elu',
                 use_residuals=False, final_pred_activation='softmax',
                 nb_conv_per_level=1, conv_dropout=0, batch_norm=None):
        super().__init__()
        ...
        for level in range(nb_levels):
            ...
            for conv in range(nb_conv_per_level):
                block = ConvBlock(
                    in_channels, nb_lvl_feats, conv_size,
                    padding=padding, activation=activation,
                    use_residual=use_residuals and conv == nb_conv_per_level - 1,
                    conv_dropout=conv_dropout, batch_norm=batch_norm,
        ...
        for level in range(nb_levels - 2, -1, -1):
            ...
            self.upsamples.append(
                upsample_layer(channels=int(nb_features * feat_mult ** (level + 1)), factor=self.pool_size)
            )
    ...
    def load_from_tensorflow(self, tf_model_path, prefix):
        ...
        for level in range(self.nb_levels):
            for conv_idx, conv_block in enumerate(self.encoder_blocks[level]):
                ...
                if tf_name_conv in tf_weights:
                    ...
                    state_dict[f'encoder_blocks.{level}.{conv_idx}.conv.bias'] = torch.from_numpy(tf_weights[tf_name_bias])
                ...
                if conv_idx == self.nb_conv_per_level - 1:  
                    ...
                    if f'{bn_prefix}/gamma:0' in tf_weights:
                        state_dict[f'encoder_blocks.{level}.{conv_idx}.batch_norm.weight'] = torch.from_numpy(tf_weights[f'{bn_prefix}/gamma:0'])
                        state_dict[f'encoder_blocks.{level}.{conv_idx}.batch_norm.bias'] = torch.from_numpy(tf_weights[f'{bn_prefix}/beta:0'])
                        state_dict[f'encoder_blocks.{level}.{conv_idx}.batch_norm.running_mean'] = torch.from_numpy(tf_weights[f'{bn_prefix}/moving_mean:0'])
                        state_dict[f'encoder_blocks.{level}.{conv_idx}.batch_norm.running_var'] = torch.from_numpy(tf_weights[f'{bn_prefix}/moving_variance:0'])
        
        for level in range(self.nb_levels - 1):
            for conv_idx, conv_block in enumerate(self.decoder_blocks[level]):
                decoder_level = self.nb_levels + level
                layer_prefix = f'{prefix}_conv_uparm_{decoder_level}_{conv_idx}/{prefix}_conv_uparm_{decoder_level}_{conv_idx}'
                tf_name_conv = f'{layer_prefix}/kernel:0'
                tf_name_bias = f'{layer_prefix}/bias:0'
                ...
                if tf_name_conv in tf_weights:
                    ...
                    state_dict[f'decoder_blocks.{level}.{conv_idx}.conv.bias'] = torch.from_numpy(tf_weights[tf_name_bias])
                
                if conv_idx == self.nb_conv_per_level - 1:  
                    bn_prefix = f'{prefix}_bn_up_{level}/{prefix}_bn_up_{level}'
                    if f'{bn_prefix}/gamma:0' in tf_weights:
                        state_dict[f'decoder_blocks.{level}.{conv_idx}.batch_norm.weight'] = torch.from_numpy(tf_weights[f'{bn_prefix}/gamma:0'])
                        state_dict[f'decoder_blocks.{level}.{conv_idx}.batch_norm.bias'] = torch.from_numpy(tf_weights[f'{bn_prefix}/beta:0'])
                        state_dict[f'decoder_blocks.{level}.{conv_idx}.batch_norm.running_mean'] = torch.from_numpy(tf_weights[f'{bn_prefix}/moving_mean:0'])
                        state_dict[f'decoder_blocks.{level}.{conv_idx}.batch_norm.running_var'] = torch.from_numpy(tf_weights[f'{bn_prefix}/moving_variance:0'])
